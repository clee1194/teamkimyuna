...
hyperparameters['eta'] = 0.00005
hyperparameters['hidden_units'] = 1000
...
def main():
	...
	drop_prob = tf.placeholder(tf.float64, [])
	...
	# h_1 = tf.nn.relu(z_1)
	z_2, loss_2 = neural_network(z_1, y_size)

	train_pred = tf.nn.relu(tf.nn.dropout(z_2, drop_prob))
	pred = tf.nn.relu(z_2 * drop_prob)
	train_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(train_pred, y_)) + hyperparameters['lambda'] * (loss_1 + loss_2)
	loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(pred, y_)) + hyperparameters['lambda'] * (loss_1 + loss_2)
	
	correct_prediction = tf.equal(tf.argmax(pred,1), tf.argmax(y_,1))
	accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float64))
	
	adam_op = tf.train.AdamOptimizer(learning_rate).minimize(train_loss)
	...
	with tf.Session() as sess:
		...
		for epoch in range(hyperparameters['num_epochs']):	
			...
			for batch in range(15000/hyperparameters['mini_batch_size']):
				...
				_, c = sess.run([adam_op, train_loss], feed_dict={x:miniData, y_:miniTarget, learning_rate:hyperparameters['eta'],
		...
		# print sess.run(tf.one_hot(testTarget[10:20], y_size))
			validError = sess.run(loss, feed_dict={x:validData, y_:validTarget, drop_prob:hyperparameters['dropout_rate']})
			testError = sess.run(loss, feed_dict={x:testData, y_:testTarget, drop_prob:hyperparameters['dropout_rate']})
			validAcc = sess.run(accuracy, feed_dict={x:validData, y_:validTarget, drop_prob:hyperparameters['dropout_rate']})
			testAcc = sess.run(accuracy, feed_dict={x:testData, y_:testTarget, drop_prob:hyperparameters['dropout_rate']})
